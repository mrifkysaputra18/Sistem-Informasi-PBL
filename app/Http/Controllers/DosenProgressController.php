...
use App\Models\{ClassRoom, Group, WeeklyTarget, WeeklyProgress};

class DosenProgressController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $classRooms = ClassRoom::with(['subject', 'groups', 'academicPeriod'])
        ...
        $stats = [
            ...
            'pendingReviews' => WeeklyProgress::whereHas('group.classRoom', function($query) use ($user) {
                $query->where('dosen_id', $user->id);
            })->where('submission_status', 'submitted')->orWhereIn('submission_status', ['submitted', 'revision'])->count(),
        ];

        return view('dosen.progress.index', compact('classRooms', 'stats'));
    }
    ...
    public function showClass(ClassRoom $classRoom)
    {
        ...
        return view('dosen.progress.show-class', compact('classRoom', 'stats'));
    }
    ...
    public function showGroup(ClassRoom $classRoom, Group $group)
    {
        ...
        return view('dosen.progress.show-group', compact('classRoom', 'group', 'stats', 'targetsWithFiles'));
    }
