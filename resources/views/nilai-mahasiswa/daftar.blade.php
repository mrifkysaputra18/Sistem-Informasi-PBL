    ...
    <div class="py-8 bg-gray-100 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <form method="GET" action="{{ route('student-scores.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['academic_year', 'class_room_id']))
                            <a href="{{ route('student-scores.index') }}" class="h-10 w-10 bg-white border-2 border-gray-300 hover:border-red-500 hover:bg-red-50 text-gray-600 hover:text-red-600 rounded-lg transition-all flex items-center justify-center" title="Reset">
                                                ...
                                                <div class="flex items-center p-2 rounded-lg {{ $index == 0 ? 'bg-yellow-50 border border-yellow-100' : ($index == 1 ? 'bg-gray-50 border border-gray-100' : ($index == 2 ? 'bg-orange-50 border border-orange-100' : '')) }}">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        ...
                                                        <div class="text-xs text-indigo-600 font-medium mt-0.5">
                                                            {{ number_format($studentData['total_score'], 1) }} poin
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <!-- Left Column: Score Matrix (Table) -->
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden flex flex-col h-full">
                        
                        <!-- TABS NAVIGATION (Server Side) -->
                        <div class="flex border-b border-gray-200 bg-gray-50 overflow-x-auto">
                            @php
                                $currentClass = request('class_room_id', 'all');
                            @endphp
                            
                            <!-- Tab: Semua -->
                            <a href="{{ route('student-scores.index', array_merge(request()->except('page'), ['class_room_id' => 'all'])) }}"
                               class="flex-shrink-0 px-6 py-4 text-sm font-bold uppercase tracking-widest transition-all border-b-4 
                               {{ $currentClass == 'all' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-100' }}">
                                <i class="fa-solid fa-users mr-2"></i> Semua
                            </a>

                            <!-- Tab: Per Kelas -->
                            @foreach($classRooms as $class)
                                <a href="{{ route('student-scores.index', array_merge(request()->except('page'), ['class_room_id' => $class->id])) }}"
                                   class="flex-shrink-0 px-6 py-4 text-sm font-bold uppercase tracking-widest transition-all border-b-4 
                                   {{ $currentClass == $class->id ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-100' }}">
                        ...
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-gray-800">
                                    <i class="fas fa-table mr-2 text-indigo-400"></i> Matriks Nilai Mahasiswa
                                </h3>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                                    <thead class="bg-gray-100">
                                        <tr>
                                            <th class="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider sticky left-0 bg-gray-100 z-20 border-r border-gray-300 shadow-sm">
                                                Mahasiswa
                                            </th>
                                            ...
                                            @endforeach
                                            <th class="px-4 py-3 text-center text-xs font-black text-indigo-800 uppercase tracking-wider bg-indigo-50 border-l border-indigo-100">
                                                Total
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200 text-gray-900">
                                        ...
                                        <tr class="hover:bg-indigo-50 transition-colors group">
                                            ...
                                            @php 
                                                ...
                                                if($studentRanking) {
                                                    $studentTotal = $studentRanking['total_score'];
                                                }
                                            @endphp
                                            ...
                                            <td class="px-4 py-3 text-center whitespace-nowrap bg-indigo-50/30 group-hover:bg-indigo-100 font-black text-indigo-900 border-l-2 border-indigo-100">
                ...
                <!-- Right Column: Ranking List (LIMITED TO TOP 10) -->
                    ...
                    <div class="p-4 space-y-3 max-h-[800px] overflow-y-auto custom-scrollbar">
                        @if(count($ranking) > 0)
                            ...
                            @foreach(collect($ranking)->take(10) as $rank)
                                <div class="flex items-center p-3 rounded-lg border transition-all hover:shadow-md bg-white
                                    {{ $rank['rank'] == 1 ? 'border-yellow-300 shadow-sm ring-1 ring-yellow-100' : ($rank['rank'] == 2 ? 'border-gray-300' : ($rank['rank'] == 3 ? 'border-orange-300' : 'border-gray-100')) }}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="text-xs text-gray-500 flex items-center gap-1">
                                            <i class="fa-solid fa-id-card text-gray-400"></i> {{ $rank['student']->nim ?? '-' }}
                                        </div>
                                    </div>
                            ...
                            @if(count($ranking) > 10)
                                <div class="text-center pt-2 pb-1">
                                    <p class="text-xs text-gray-500 italic">... dan {{ count($ranking) - 10 }} mahasiswa lainnya</p>
                                </div>
                            @endif
                        @else
