{{-- View: Daftar Ranking & Penilaian Kelompok | Controller: NilaiKelompokController@index --}}
<x-app-layout>
    <div class="py-8 bg-gray-100 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                ...
                {{-- Tombol hitung ulang ranking (hanya admin) --}}
            ...
            {{-- Tab Mahasiswa/Kelompok Terbaik --}}
            <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden mb-8" x-data="{ activeTab: 'students' }">
                {{-- Header Tab --}}
                <div class="flex border-b border-gray-200 bg-gray-50">
                    <button @click="activeTab = 'students'" :class="activeTab === 'students' ? 'border-indigo-600 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-100'" class="flex-1 py-4 text-sm font-bold uppercase tracking-widest transition-all border-b-4">
                        <i class="fa-solid fa-user-graduate mr-2"></i> Mahasiswa Terbaik
                    </button>
                    <button @click="activeTab = 'groups'" :class="activeTab === 'groups' ? 'border-indigo-600 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-100'" class="flex-1 py-4 text-sm font-bold uppercase tracking-widest transition-all border-b-4">
                                            ...
                                            @foreach($classData['top_students'] as $index => $studentData)
                                                <div class="flex items-center p-2 rounded-lg {{ $index == 0 ? 'bg-yellow-50 border border-yellow-100' : ($index == 1 ? 'bg-gray-50 border border-gray-100' : ($index == 2 ? 'bg-orange-50 border border-orange-100' : '')) }}">
                                                    ...
                                                    <div class="text-right pl-2">
                                                        <span class="block text-sm font-black {{ $index == 0 ? 'text-yellow-600' : 'text-indigo-600' }}">{{ number_format($studentData['final_score'], 1) }}</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="p-2 space-y-2">
                                            @foreach($classData['top_groups'] as $index => $groupData)
                                                <div class="flex items-center p-2 rounded-lg {{ $index == 0 ? 'bg-yellow-50 border border-yellow-100' : '' }}">
                                                    ...
                                                    <div class="text-right pl-2">
                                                        <span class="block text-sm font-black text-indigo-600">{{ number_format($groupData['total_score'], 2) }}</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
            ...
            {{-- Matriks Penilaian & Ranking Global --}}
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 z-10 border-r border-gray-200 shadow-sm">Kelompok</th>
                                    ...
                                    @endforeach
                                    <th class="px-4 py-3 text-center text-xs font-black text-indigo-600 uppercase tracking-wider bg-indigo-50">Total</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-indigo-50 transition-colors group">
                                    ...
                                    @foreach($criteria as $criterion)
                                        ...
                                        <td class="px-4 py-3 text-center whitespace-nowrap">
                                            @if($score)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $nilai >= 80 ? 'bg-green-100 text-green-800' : ($nilai >= 60 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">{{ number_format($nilai, 0) }}</span>
                                            @else
                                                <span class="text-gray-300">-</span>
                                            @endif
                                        </td>
                                    @endforeach
                                    <td class="px-4 py-3 text-center whitespace-nowrap bg-indigo-50/50 font-black text-indigo-700">{{ number_format($totalScore, 2) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="p-4 space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar">
                        @if(count($ranking) > 0)
                            @foreach($ranking as $rank)
                                <div class="flex items-center p-3 rounded-lg border transition-all hover:shadow-md {{ $rank['rank'] == 1 ? 'bg-yellow-50 border-yellow-200' : ($rank['rank'] == 2 ? 'bg-gray-50 border-gray-200' : ($rank['rank'] == 3 ? 'bg-orange-50 border-orange-200' : 'bg-white border-gray-100')) }}">
