                        ...
                        @if(in_array(auth()->user()->role, ['dosen', 'admin', 'koordinator']) && !$target->isReviewed())
                            @if($target->isClosed())
                                ...
                                <form action="{{ route('targets.reopen', $target->id) }}" 
                                    ...
                                    @csrf
                                    <button type="submit" 
                                            class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105">
