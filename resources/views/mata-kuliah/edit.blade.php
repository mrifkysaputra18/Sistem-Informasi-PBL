            ...
            <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                <form action="{{ route('mata-kuliah.update', $mataKuliah) }}" method="POST" class="p-8">
                    ...
                    <div class="space-y-6">
                        ...
                        @php
                            $dosenSebelumUtsId = $mataKuliah->dosens->where('pivot.periode', 'sebelum_uts')->first()?->id;
                            $dosenSesudahUtsId = $mataKuliah->dosens->where('pivot.periode', 'sesudah_uts')->first()?->id;
                        @endphp
                        ...
                        <div>
                            ...
                            <select name="dosen_sebelum_uts_id" id="dosen_sebelum_uts_id"
                                ...
                                @foreach($dosens as $dosen)
                                    <option value="{{ $dosen->id }}" {{ old('dosen_sebelum_uts_id', $dosenSebelumUtsId) == $dosen->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="dosen_sesudah_uts_id" id="dosen_sesudah_uts_id"
                                ...
                                <option value="">-- Pilih Dosen (Optional) --</option>
                                @foreach($dosens as $dosen)
                                    <option value="{{ $dosen->id }}" {{ old('dosen_sesudah_uts_id', $dosenSesudahUtsId) == $dosen->id ? 'selected' : '' }}>
                                        {{ $dosen->name }} - {{ $dosen->email }}
                                    </option>
                                @endforeach
                            </select>
