                ...
                <form action="{{ route('rubrik-penilaian.store', $mataKuliah) }}" method="POST" class="p-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="border-t-2 border-gray-200 pt-6">
                            ...
                            <!-- Template dari rubrik yang sudah ada -->
                            @if($existingRubriks->count() > 0)
                            <div class="mb-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                                <label class="block text-sm font-bold text-orange-800 mb-2">
                                    <i class="fa-solid fa-copy mr-1"></i> Gunakan Template dari Rubrik yang Ada
                                </label>
                                <select @change="loadTemplate($event.target.value)" class="w-full px-3 py-2 border border-orange-300 rounded-lg text-sm">
                                    <option value="">-- Pilih Template (Opsional) --</option>
                                    @foreach($existingRubriks as $rubrik)
                                        <option value="{{ $rubrik->id }}" data-items='@json($rubrik->items)'>
                                            {{ $rubrik->nama }} ({{ $rubrik->periodeAkademik->name ?? '' }} - Smt {{ $rubrik->semester }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <script>
        function rubrikForm() {
            return {
                ...
                addItem() {
                    this.items.push({ nama: '', persentase: '', deskripsi: '' });
                },
                removeItem(index) {
                ...
                loadTemplate(rubrikId) {
                    if (!rubrikId) return;
                    const select = document.querySelector('select[onchange]') || event.target;
                    const option = select.querySelector(`option[value="${rubrikId}"]`);
                    if (option && option.dataset.items) {
                        const templateItems = JSON.parse(option.dataset.items);
                        this.items = templateItems.map(item => ({
                            nama: item.nama,
